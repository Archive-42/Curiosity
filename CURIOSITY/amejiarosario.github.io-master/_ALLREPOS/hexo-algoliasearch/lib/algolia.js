...
function algolia(args, callback) {
  ...
  actions.year = (string) => new Date(string).getFullYear();

  actions.yearMonth = (string) => {
    const d = new Date(string);
    return `${d.getFullYear()}-${d.toString().substring(4,7)}`
  };

  actions.thousands = roundNumbers(1e3);
  actions.hundreds = roundNumbers(1e2);
  actions.hundredThousands = roundNumbers(1e5);
  function roundNumbers(base){
    return function(number){
      return Math.round(parseInt(number, 10)/base)*base;
    }
  }
