...
def harrison_ford
  ...
  Movie.select(:id, :title).joins(castings: :actor).where("castings.ord != 1 AND actors.name = 'Harrison Ford'")
end
...
def directed_by_one_of(them)
  ...
  # Movie.select(:id, :title).joins(castings: :actor).where("actors.name IN (?)", them)
    director_ids = Actor.select(:id).where(name: them)
    Movie.select(:id, :title).where(director_id: director_ids)
end
