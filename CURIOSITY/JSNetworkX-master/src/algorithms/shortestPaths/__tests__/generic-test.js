...
import { Graph, DiGraph, Map, convertNodeLabelsToIntegers } from "../../../";

import { cycleGraph, grid2dGraph, pathGraph } from "../../../generators";

import { hasPath, shortestPath, shortestPathLength } from "../generic";
...
export var testGenericPath = {
  ...
  testShortestPathLength: function () {
    assert.equal(shortestPathLength(this.cycle, { source: 0, target: 3 }), 3);
    assert.equal(shortestPathLength(this.grid, { source: 1, target: 12 }), 5);
    assert.equal(
  ...
  testSingleSourceShortestPath: function () {
    ...
    assert.deepEqual(
      paths,
      singleSourceDijkstraPath(this.cycle, { source: 0 })
    );
  ...
  testAllPairsShortestPathLength: function () {
    ...
    assert.deepEqual(distances, allPairsDijkstraPathLength(this.cycle));
