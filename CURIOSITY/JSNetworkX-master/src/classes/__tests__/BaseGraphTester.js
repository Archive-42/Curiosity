...
export default {
  ...
  testNodesIter: function () {
    ...
    assert.sameMembersDeep(Array.from(G.nodesIter(true)), [
      [0, {}],
      [1, {}],
      [2, {}],
    ]);
  ...
  testEdgesIter: function () {
    ...
    assert.deepEqual(Array.from(G.edgesIter()).sort(), [
      [0, 1],
      [0, 2],
      [1, 2],
    ]);
    assert.deepEqual(Array.from(G.edgesIter(0)).sort(), [
      [0, 1],
      [0, 2],
    ]);
    assert.throws(function () {
  ...
  testAddPath: function () {
    ...
    var nlist = [12, 13, 14, 15];
    G.addPath(nlist);
    assert.deepEqual(Array.from(G.edges(nlist)), [
      [12, 13],
      [13, 14],
      [14, 15],
    ]);
