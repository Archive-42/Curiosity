...
export default class DiGraph extends Graph {
  ...
  *inEdgesIter(optNbunch, optData = false) {
  ...
  degreeIter(optNbunch, optWeight) {
    ...
    if (optNbunch == null) {
      ...
      nodesNbrs = zipIterator(
        mapIterator(this.nbunchIter(optNbunch), (n) =>
          tuple2Succ(n, this.succ.get(n))
        ),
        mapIterator(this.nbunchIter(optNbunch), (n) =>
          tuple2Pred(n, this.pred.get(n))
        )
      );
    }
    ...
    if (optWeight == null) {
      ...
      return mapIterator(nodesNbrs, ([[node, succ], [u, pred]]) => [
        node,
        pred.size + succ.size,
      ]);
    } else {
      // edge weighted graph - degree is sum of edge weights
      return mapIterator(nodesNbrs, ([[node, succ], [_, pred]]) => {
        var sum = 0;
  ...
  inDegreeIter(optNbunch, optWeight) {
    ...
    if (optWeight == null) {
      return mapIterator(nodesNbrs, ([node, pred]) => [node, pred.size]);
    } else {
      return mapIterator(nodesNbrs, function ([node, pred]) {
        var sum = 0;
  ...
  outDegreeIter(optNbunch, optWeight) {
    ...
    if (optNbunch == null) {
      ...
      nodesNbrs = mapIterator(this.nbunchIter(optNbunch), (n) =>
        tuple2(n, this.succ.get(n))
      );
    }

    if (optWeight == null) {
      return mapIterator(nodesNbrs, ([node, succ]) => [node, succ.size]);
    } else {
      return mapIterator(nodesNbrs, function ([node, succ]) {
        var sum = 0;
