...
function singleSourceShortestPathLength(G, source, optCutoff) {
  ...
  while (nextlevel.size > 0) {
    ...
    try {
      for (
        var _iterator = _getIterator(thislevel.keys()), _step;
...
function genSingleSourceShortestPathLength(G, source, optCutoff) {
  return (0, _internalsDelegate2["default"])("singleSourceShortestPathLength", [
    G,
    source,
    optCutoff,
  ]);
...
function genAllPairsShortestPathLength(G, optCutoff) {
  return (0, _internalsDelegate2["default"])("allPairsShortestPathLength", [
    G,
    optCutoff,
  ]);
}
...
function genBidirectionalShortestPath(G, source, target) {
  return (0, _internalsDelegate2["default"])("bidirectionalShortestPath", [
    G,
    source,
    target,
  ]);
...
function bidirectionalPredSucc(G, source, target) {
  ...
  if ((0, _internals.nodesAreEqual)(source, target)) {
    return [
      new _internals.Map([[source, null]]),
      new _internals.Map([[target, null]]),
      source,
    ];
  }
  ...
  while (forwardFringe.length > 0 && reverseFringe.length > 0) {
    if (forwardFringe.length <= reverseFringe.length) {
      ...
      try {
        for (
          var _iterator3 = _getIterator(thisLevel), _step3;
          ...
          try {
            for (
              var _iterator4 = _getIterator(gsucc(v)), _step4;
      ...
      try {
        for (
          var _iterator5 = _getIterator(thisLevel), _step5;
          ...
          try {
            for (
              var _iterator6 = _getIterator(gpred(v)), _step6;
  ...
  throw new _exceptions.JSNetworkXNoPath(
    (0, _internals.sprintf)("No path between `%j` and `%j`.", source, target)
  );
}
...
function singleSourceShortestPath(G, source, optCutoff) {
  ...
  while (nextlevel.size > 0) {
    ...
    try {
      for (
        var _iterator7 = _getIterator(thislevel.keys()), _step7;
        ...
        try {
          for (
            var _iterator8 = _getIterator(G.get(v).keys()), _step8;
...
function genSingleSourceShortestPath(G, source, optCutoff) {
  return (0, _internalsDelegate2["default"])("singleSourceShortestPath", [
    G,
    source,
    optCutoff,
  ]);
...
function allPairsShortestPath(G, optCutoff) {
  ...
  try {
    for (
      var _iterator9 = _getIterator(G), _step9;
...
function genAllPairsShortestPath(G, optCutoff) {
  return (0, _internalsDelegate2["default"])("allPairsShortestPath", [
    G,
    optCutoff,
  ]);
}
...
function genPredecessor(G, source, optArgs) {
  return (0, _internalsDelegate2["default"])("predecessor", [
    G,
    source,
    optArgs,
  ]);
