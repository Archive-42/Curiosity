...
function assertSameNodes(G, H) {
  ...
  if (
    Hnodes.size !== Gnodes.size ||
    (0, _internals.someIterator)(Gnodes.values(), function (v) {
      return !Hnodes.has(v);
    })
  ) {
    throw new _exceptionsJSNetworkXError2["default"](
      "Node sets of graphs are not equal."
    );
  }
}
...
function union(G, H) {
  ...
  if (
    new _internalsSet2["default"](G).intersection(
...
function difference(G, H) {
  ...
  try {
    for (
      var _iterator2 = _getIterator(edges), _step2;
...
function symmetricDifference(G, H) {
  ...
  assertSameNodes(G, H);
  R.addNodesFrom(
    (0, _internalsSet.symmetricDifference)(
  ...
  try {
    for (
      var _iterator3 = _getIterator(edges), _step3;
