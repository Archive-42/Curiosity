...
export async function dijkstraPath(G, { source, target, weight = "weight" }) {
  var [distances, paths] = await singleSourceDijkstra(G, { // eslint-disable-line no-unused-vars
    source,
    target,
    weight,
  });
...
export async function singleSourceDijkstraPath(
  ...
  var [length, path] = await singleSourceDijkstra(G, { // eslint-disable-line no-unused-vars
    source,
    cutoff,
    weight,
  });
