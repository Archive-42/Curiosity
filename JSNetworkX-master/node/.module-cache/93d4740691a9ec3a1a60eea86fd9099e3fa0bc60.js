...
var marked0$0 = [trianglesAndDegreeIter, weightedTrianglesAndDegreeIter].map(
...
function triangles(G, optNodes) {
  ...
  if (optNodes != null && G.hasNode(optNodes)) {
    // return single value
    return Math.floor(
      (0, _internals.next)(trianglesAndDegreeIter(G, optNodes))[2] / 2
    );
  }

  return new _internals.Map(
...
function trianglesAndDegreeIter(G, optNodes) {
  ...
  return _regeneratorRuntime.wrap(
    function trianglesAndDegreeIter$(context$1$0) {
      while (1)
...
function weightedTrianglesAndDegreeIter(G, optNodes) {
  ...
  return _regeneratorRuntime.wrap(
    function weightedTrianglesAndDegreeIter$(context$1$0) {
      while (1)
        switch ((context$1$0.prev = context$1$0.next)) {
          case 0:
            if (!G.isMultigraph()) {
          ...
          case 2:
            maxWeight =
              optWeight == null || G.edges().length === 0
                ? 1
                : (0, _internals.max)(
                    (0, _internals.mapIterator)(
                      G.edgesIter(true),
          ...
          case 21:
            ...
            j = _step4.value;
            weightij =
              (0, _internals.getDefault)(nbrs.get(j)[optWeight], 1) / maxWeight;
            ...
            for (
              _iterator5 = _getIterator(inbrs.intersection(jnbrs));
              !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done);
              _iteratorNormalCompletion5 = true
            ) {
              k = _step5.value;
              weightjk =
                (0, _internals.getDefault)(G.get(j).get(k)[optWeight], 1) /
                maxWeight;
              weightki =
                (0, _internals.getDefault)(nbrs.get(k)[optWeight], 1) /
                maxWeight;

              weightedTriangles += Math.pow(
                weightij * weightjk * weightki,
                1 / 3
              );
            }
            context$1$0.next = 37;
...
function genAverageClustering(G, optNodes, optWeight, optCountZeros) {
  return (0, _internalsDelegate2["default"])("averageClustering", [
    G,
    optNodes,
    optWeight,
    optCountZeros,
...
function clustering(G, optNodes, optWeight) {
  ...
  var clusters = new _internals.Map(
    (0, _internals.mapIterator)(trianglesIter, function (_ref4) {
      ...
      return (0,
      _internals.tuple2)(node, triangles === 0 ? 0 : triangles / (degree * (degree - 1)));
    })
  );

  return G.hasNode(optNodes)
    ? (0, _internals.next)(clusters.values())
    : clusters;
}
...
function genClustering(G, optNodes, optWeight) {
  return (0, _internalsDelegate2["default"])("clustering", [
    G,
    optNodes,
    optWeight,
  ]);
...
function transitivity(G) {
  ...
  try {
    for (
      var _iterator6 = _getIterator(trianglesAndDegreeIter(G)), _step6;
...
function squareClustering(G, optNodes) {
  ...
  try {
    for (
      var _iterator7 = _getIterator(nodesIter), _step7;
      ...
      try {
        for (
          var _iterator8 = _getIterator(
              (0, _internals.genCombinations)(G.get(v).keys(), 2)
            ),
            _step8;
          ...
          var squares = new _internals.Set(G.get(u).keys()).intersection(
            new _internals.Set(G.get(w).keys())
          );
          squares["delete"](v);
          ...
          potential +=
            (G.get(u).size - degm) * (G.get(w).size - degm) + squares;
        }
      } catch (err) {
