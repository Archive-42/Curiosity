...
function eigenvectorCentrality(G) {
  ...
  if (!nstart) {
    ...
    try {
      for (
        var _iterator2 = _getIterator(x.keys()), _step2;
  ...
  try {
    for (
      var _iterator3 = _getIterator(x.values()), _step3;
  ...
  try {
    for (
      var _iterator4 = _getIterator(x), _step4;
  ...
  for (var i = 0; i < maxIter; i++) {
    ...
    try {
      for (
        var _iterator5 = _getIterator(x), _step5;
        ...
        try {
          for (
            var _iterator8 = _getIterator(G.get(n)), _step8;
            ...
            x.set(
              ...
              x.get(nbr) +
                xlast.get(n) *
                  (0, _internals.getDefault)(weight && data[weight], 1)
            );
          }
        } catch (err) {
    ...
    try {
      for (
        var _iterator6 = _getIterator(x.values()), _step6;
    ...
    try {
      for (
        var _iterator7 = _getIterator(x), _step7;
...
function genEigenvectorCentrality(G, _maxIter$tolerance$nstart$weight) {
  return (0, _internalsDelegate2["default"])("eigenvectorCentrality", [
    G,
    _maxIter$tolerance$nstart$weight,
  ]);
}
