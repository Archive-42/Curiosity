...
"use strict";
{
  Object.assign( self, {edr, edm});

  test_ed();

  function edr(n, k) {
    const trials = 1 + Math.min(
      ...Array(k+1).join('.').split('').map( (_,i) => i ).map(
        x => Math.max( edr( n - 1, x - 1 ), edr( n, k - x ) )
      )
    );
    return trials;
  }
  ...
  function edm(n, k, m = {}) {
    ...
    if ( Number.isInteger( m[key] ) ) {
      ...
      const trials = Array(k+1).join('.').split('').map( (_,i) => i + 1 ).map(
          x => Math.max( edm( n - 1, x - 1, m ), edm( n, k - x, m ) )
        );
  ...
  function test_ed() {
    ...
    tests.forEach( ([n, k, r]) => console.log( 
      edm(n,k) == r ? `EDM ${n} ${k} == ${r} passed` :
      `EDM ${n} ${k} == ${r} failed`
    ));
  }
}
