...
def heart_tracks
  ...
  execute(<<-SQL)
    SELECT
      albums.title, COUNT(tracks.*) AS heart_tracks
    FROM
      albums
    JOIN
      tracks ON tracks.album = albums.asin
...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      styles.style,
      (SUM(num_tracks_per_album.price) / SUM(num_tracks_per_album.count))
    FROM
      styles
    JOIN (
      SELECT
      ...
      GROUP BY
        albums.asin
      ) AS num_tracks_per_album ON styles.album = num_tracks_per_album.asin
    GROUP BY
      styles.style
    ORDER BY
      (SUM(num_tracks_per_album.price) / SUM(num_tracks_per_album.count)) DESC,
      styles.style ASC
    LIMIT 5
  SQL
end
