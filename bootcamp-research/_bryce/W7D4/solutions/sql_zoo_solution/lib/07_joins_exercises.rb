...
def andrews_films_and_leads
  execute(<<-SQL)
    ...
    JOIN
      actors ON actors.id = castings.actor_id
    WHERE
      movies.id IN ( -- Subquery returns all movies that Julie Andrews is in
        SELECT
          movies.id
        FROM
          movies -- Do not have to alias, scope is contained within subquery (vs. movies from 120)
        JOIN
          castings ON castings.movie_id = movies.id
        JOIN
          actors ON actors.id = castings.actor_id
      ...
      ) AND castings.ord = 1 -- Restricts the actor to lead, not necessarily Julie
    ORDER BY
      movies.title;
  SQL
end
...
def colleagues_of_garfunkel
  ...
  execute(<<-SQL)
    SELECT
      a1.name
    FROM ( -- Every movie where Art Garfunkel is in
        SELECT
          movies.*
        FROM
          movies
    ...
    JOIN ( -- Actors name not 'Art Garfunkel'
        SELECT
          actors.*,
          castings.movie_id -- We need to select here so we can join on it
        FROM
          actors
        JOIN
          castings ON castings.actor_id = actors.id
