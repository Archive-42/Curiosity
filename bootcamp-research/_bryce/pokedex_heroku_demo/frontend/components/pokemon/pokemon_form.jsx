...
class PokemonForm extends React.Component {
	...
	handleSubmit(e) {
		e.preventDefault();
		this.props.createPokemon(this.state).then(data => this.props.history.push(`/pokemon/${data.pokemon.id}`));
	}
	...
	render() {
		return (
			<section className="pokemon-detail">
				<img src={window.pokemonLogo} alt="Copyright of Nintendo Pokemon" />
				<ul>{this.errors()}</ul>
				<form className="pokemon-form" onSubmit={this.handleSubmit}>
					...
					<select value={this.state.type} onChange={this.update('poke_type')} defaultValue="Select Pokemon Type">
