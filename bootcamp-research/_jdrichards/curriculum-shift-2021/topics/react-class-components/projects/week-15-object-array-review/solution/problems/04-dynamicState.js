...
const dynamicState = (state, action, data) => {
    const newState = { session: { ...state.session } };
    ...
    switch (action) {
        ...
        case 'delete':
            delete newState.session[data]
            return newState;

        case 'replace':
            const keyReplace = Object.keys(data)[0];
            newState.session[keyReplace] = data[keyReplace];
            return newState;
