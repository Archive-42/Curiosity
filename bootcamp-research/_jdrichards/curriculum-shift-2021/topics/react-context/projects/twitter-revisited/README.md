...
To set up the proxy, you need to add the following line in your `package.json`.
...
`user.id` from your fetch response, once it is parsed in JSON. In the previous
frontend application, you simply used `localStorage` to store a user's access
token and keep track of the current user's ID. In a later phase, you'll use
[React Context] with `localStorage` to share these values across different
components.
...
`authToken` and `currentUserId` in state, you can persist user login by having
the default state read from `localStorage`.
...
browser's `localStorage`, those values will be set as the default state. If your
browser's `localStorage` does not have an `authToken` or a `currentUserId`, then
...
`AppWithContext`. You'll also use the method's arguments to set an `authToken`
and `currentUserId` in your browser's `localStorage` to be able to persist user
login.
...
component. You'll also want your method to set the `authToken` and
`currentUserId` as items in `localStorage` so that your user login can be
persisted.

If you don't use `localStorage` to persist the `authToken` and `currentUserId`
in your browser, you will be logged out upon refresh. When you refresh your
browser, your application's context will reset to the default state of
`AppWithContext` (where `authToken` and `currentUserId` are `null`).
...
Instead of using the `token` and `id` values to set items in `localStorage`,
you'll invoke the `updateContext` method passed as a prop to update the state of
`AppWithContext` _and_ set the items in `localStorage`.
